...
import com.workworth.workday.api.dto.*; import com.workworth.workday.application.WorkdayService; import java.time.Clock; import java.time.LocalDate; import org.springframework.http.ResponseEntity; import org.springframework.validation.annotation.Validated; import org.springframework.web.bind.annotation.*;
@RestController @RequestMapping("/api/v1/workdays") @Validated public class WorkdayController {
 private final WorkdayService service; private final Clock clock; public WorkdayController(WorkdayService service, Clock clock){this.service=service;this.clock=clock;}
 ...
 @GetMapping("/current") public ResponseEntity<WorkdayResponse> current(){return get(LocalDate.now(clock));}
 ...
 @PutMapping("/{date}/meal-breaks/{id}") public ResponseEntity<MealBreakResponse> amend(@PathVariable LocalDate date,@PathVariable Long id,@RequestBody @jakarta.validation.Valid UpdateMealBreakRequest r){return ResponseEntity.ok(MealBreakResponse.from(service.amendHistoricalMealBreak(date,id,r.startedAt(),r.endedAt())));}
 ...
 @PutMapping("/{date}/partial-absences/{id}") public ResponseEntity<PartialAbsenceResponse> updateAbsence(@PathVariable LocalDate date,@PathVariable Long id,@RequestBody @jakarta.validation.Valid CreatePartialAbsenceRequest r){return ResponseEntity.ok(PartialAbsenceResponse.from(service.updateAbsence(date,id,r.startedAt(),r.endedAt(),r.reason())));}
