...
public class Workday {
    ...
    @Column(name = "schedule_variant", nullable = false, length = 16)
    private ScheduleVariant scheduleVariant;
    @Column(name = "scheduled_start", nullable = false)
    private LocalTime scheduledStart;
    @Column(name = "scheduled_end", nullable = false)
    private LocalTime scheduledEnd;
    @Column(name = "maximum_economic_seconds", nullable = false)
    private long maximumEconomicSeconds;
    @Enumerated(EnumType.STRING)
    ...
    public Workday(LocalDate date, String zone, ScheduleVariant variant, LocalTime start, LocalTime end, long maxSeconds, Instant now) {
        ...
        scheduledEnd = end;
        maximumEconomicSeconds = maxSeconds;
        status = WorkdayStatus.SCHEDULED;
        createdAt = now;
        updatedAt = now;
    }

    public void changeStatus(WorkdayStatus value, Instant now) {
        status = value;
        updatedAt = now;
    }

    public void cancel(String reason, Instant now) {
        status = WorkdayStatus.CANCELLED;
